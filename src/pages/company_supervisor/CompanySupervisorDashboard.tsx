...
function CompanyDashboardContent() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* top 3 stat cards — même grid que l'original */}
          ...
          {loadingProjects ? <Spinner /> : (
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {projects.reduce((a, p) => a + p.contributors.length, 0)} interns total
              </p>
            </div>
          )}
        </div>
      ...
      {/* big main block — même que l'original */}
      <div className="min-h-[100vh] flex-1 rounded-xl bg-muted/50 md:min-h-min p-6 flex flex-col gap-6">
              ...
              {projects.map((p) => (
                ...
                >
                  ...
                  <span className={`ml-1.5 text-[10px] rounded-full px-1.5 py-0.5 ${
                    selectedId === p._id ? "bg-white/20" : "bg-muted"
                  }`}>
                ...
                <div className="rounded-lg bg-background/60 px-3 py-2">
                  <div className="flex justify-between text-sm mb-1.5">
                    ...
                    <span className="text-muted-foreground text-xs">
                      {progress.projectProgress.doneTasks}/{progress.projectProgress.totalTasks} · {progress.projectProgress.progress}%
                    </span>
                  </div>
                  <ProgressBar value={progress.projectProgress.progress} />
                </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
                ...
                {validations.map((t) => (
                  <div key={t._id} className="rounded-lg border-l-2 border-yellow-400 bg-background/60 px-3 py-2">
                    ...
                    {t.lastValidation ? (
                      ...
                      <span className="inline-block mt-1 text-[10px] font-semibold bg-yellow-100 text-yellow-700 rounded-full px-2 py-0.5">
                        No validation yet
                      </span>
                    )}
                  </div>
          ...
          <div>
            ...
            {loadingMeetings ? <Spinner /> : meetings.length === 0 ? (
              <Empty msg="No meetings scheduled." />
            ) : (
              <div className="flex flex-col gap-2">
                {meetings.map((m) => (
        ...
        <div>
          ...
          {loadingTimeline ? <Spinner /> : timeline.length === 0 ? (
            <Empty msg="No activity this month." />
          ) : (
